...
`;
  ...
  async start(): Promise<void> {
    ...
    // Check if SAM is already available (i2pd running externally)
    const samAlreadyAvailable = await this.isSAMAvailable();
    if (samAlreadyAvailable) {
      console.log('[I2PD] SAM bridge already available (external i2pd detected)');
